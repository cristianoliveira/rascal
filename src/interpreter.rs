...
use ast::{Node, Operation, Var, Frame};
use parser::Parser;
...
impl Interpreter {
    ...
    pub fn pop(&mut self) -> Frame {
        ...
        for (k, v) in old.locals.iter() {
            let curr = self.scope().clone();
            if let Some(var) = curr.locals.get(k) {
                self.scope().locals.insert(k.clone(),v.clone());
            }
        };
        for (k, v) in old.ilocals.iter() {
            let curr = self.scope().clone();
            if let Some(var) = curr.ilocals.get(k) {
                self.scope().ilocals.insert(k.clone(),v.clone());
            }
        };
        println!("stack: {:?}", self.stack);
        old
    ...
    pub fn eval_tree(&mut self, tree: Node) -> String {
        ...
        match *operation.clone() {
            ...
            Operation::Block(statements) => {
                let mut block_scope = Frame::new();
                self.push(&block_scope);
